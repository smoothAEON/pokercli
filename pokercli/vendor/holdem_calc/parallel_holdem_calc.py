...
def run_simulation(hole_cards, num, exact, given_board, deck, verbose):
    ...
    if (None, None) in hole_cards:
        ...
        find_winner(generate_boards, deck, hole_cards, num, board_length, given_board, winner_list, result_histograms)
    ...
    for index, element in enumerate(result_histograms):
        combined_histograms[(index // num_poker_hands) % num_players][index % num_poker_hands] += element
    if verbose:
...
def unknown_simulation(new_hole_cards):
    ...
    holdem_functions.find_winner(generate_boards, deck, tuple(hole_cards_list), num, board_length, given_board, winner_list, result_histograms)
    ...
    for histogram_index, histogram in enumerate(result_histograms):
        for index, result in enumerate(histogram):
            combined_result_histograms[len(holdem_functions.hand_rankings) * (proc_id * num_players + histogram_index) + index] += result
...
def find_winner(generate_boards, deck, hole_cards, num, board_length, given_board, winner_list, result_histograms):
    ...
    pool = multiprocessing.Pool(
        ...
        initargs=(given_board, hole_cards, winner_list, result_histograms),
    )
...
def simulation(remaining_board):
    ...
    proc_name = multiprocessing.current_process().name
    proc_id = int(proc_name.split("-")[-1]) % multiprocessing.cpu_count()
    result_list = [[] for _ in range(num_players)]
    ...
    for index, result in enumerate(result_list):
        result_histograms[len(holdem_functions.hand_rankings) * (proc_id * num_players + index) + result[0]] += 1
